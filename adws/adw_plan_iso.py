...
def main():
    ...
    # Claude Code v2.0.30+ returns verbose output, so extract the file path
    logger.info("Extracting plan file path from response")
    plan_file_path = extract_plan_file_path(plan_response.output)

    # Fallback to treating entire output as path if extraction fails
    if not plan_file_path:
        logger.warning("Could not extract file path from verbose output, using raw output")
    ...
    if not os.path.exists(worktree_plan_path):
        # File not in worktree - check if it was written to main repo instead
        # This happens when issue body contains explicit main repo paths
        project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        main_repo_plan_path = os.path.join(project_root, plan_file_path)

        if os.path.exists(main_repo_plan_path):
            logger.warning(f"Plan file found in main repo instead of worktree, moving to worktree")
            logger.debug(f"Source: {main_repo_plan_path}")
            logger.debug(f"Destination: {worktree_plan_path}")

            # Ensure destination directory exists
            os.makedirs(os.path.dirname(worktree_plan_path), exist_ok=True)

            # Move file from main repo to worktree
            shutil.move(main_repo_plan_path, worktree_plan_path)

            logger.info(f"Successfully moved plan file to worktree: {worktree_plan_path}")
            make_issue_comment(
                issue_number,
                format_issue_message(adw_id, "ops", f"⚠️ Plan file was created in main repo, moved to worktree: {plan_file_path}"),
            )
        else:
            error = f"Plan file does not exist in worktree or main repo: {plan_file_path}"
            logger.error(error)
            logger.debug(f"Checked worktree path: {worktree_plan_path}")
            logger.debug(f"Checked main repo path: {main_repo_plan_path}")
            make_issue_comment(
                issue_number,
                format_issue_message(adw_id, "ops", f"❌ {error}"),
            )
