...
def create_branch(
    ...
    # Create branch
    result = subprocess.run(
        ["git", "checkout", "-b", branch_name], capture_output=True, text=True, encoding='utf-8', cwd=cwd
    )
    if result.returncode != 0:
...
def commit_changes(
    ...
    # Stage all changes
    result = subprocess.run(
        ["git", "add", "-A"], capture_output=True, text=True, encoding='utf-8', cwd=cwd
    )
    if result.returncode != 0:
        return False, result.stderr
    ...
    # Commit
    result = subprocess.run(
        ["git", "commit", "-m", message], capture_output=True, text=True, encoding='utf-8', cwd=cwd
    )
    if result.returncode != 0:
        return False, result.stderr
    return True, None
